...
export const HistoryModal: React.FC<HistoryModalProps> = ({
  ...
  const totalRevenue = filteredHistory.reduce((s, t) => s + Number(t.total_harga), 0);
  ...
  // Monthly summary cards (Rekap tab)
  const monthlyRekap = useMemo(() => {
    ...
    history.forEach(trx => {
      ...
      months[monthKey].items += trx.items?.reduce((s, item) => s + item.jumlah, 0) || 0;
  ...
  // ---------------------------------------------------------------------------
  // Skeleton row — shown while full history is loading
  // ---------------------------------------------------------------------------
  const SkeletonRow = () => (
    <div className="bg-white rounded-3xl border border-slate-100 p-4 md:p-6 animate-pulse">
      <div className="flex items-center gap-4">
        <div className="w-12 h-12 rounded-2xl bg-slate-100 shrink-0" />
        <div className="flex-1 space-y-2">
          <div className="h-3 bg-slate-100 rounded-full w-32" />
          <div className="h-5 bg-slate-100 rounded-full w-48" />
        </div>
      ...
      >
        ...
        <div className="p-4 md:p-8 border-b flex flex-col md:flex-row justify-between items-start md:items-center gap-4 bg-white relative">
          <div className="flex items-center gap-4 pr-10">
            ...
            {/* Loading indicator shown while full archive is being fetched */}
            {isLoadingHistory && (
          ...
          <div className="flex bg-slate-100 p-1.5 rounded-2xl gap-1 w-full md:w-auto">
            <button
              ...
              className={`flex-1 md:flex-none px-6 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${
                activeTab === 'list' ? 'bg-white text-blue-600 shadow-sm' : 'text-slate-400 hover:text-slate-600'
              }`}
            >
            ...
            <button
              ...
              className={`flex-1 md:flex-none px-6 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${
                activeTab === 'rekap' ? 'bg-white text-blue-600 shadow-sm' : 'text-slate-400 hover:text-slate-600'
              }`}
            >
              <div className="flex items-center justify-center gap-2">
          ...
          <button
            onClick={onClose}
            className="absolute right-4 top-4 md:relative md:right-auto md:top-auto p-2 hover:bg-slate-100 rounded-full transition-all active:scale-90 md:ml-4"
          >
        ...
        <div className="flex-1 overflow-hidden flex flex-col bg-slate-50/30">
          ...
          {activeTab === 'list' ? (
            <>
              ...
              <div className="px-4 md:px-8 py-4 border-b border-slate-100 flex flex-col md:flex-row items-center gap-4 md:gap-6 bg-white/50 backdrop-blur-md">
                ...
                <div className="text-[10px] font-black text-slate-400 uppercase tracking-widest whitespace-nowrap self-end md:self-auto">
                  {isLoadingHistory ? 'Memuat data...' : `${filteredHistory.length} Record Ditemukan`}
                </div>
              </div>
                    ...
                    <div className="text-right flex-shrink-0">
                      <div className="inline-flex items-center gap-1.5 bg-white/10 border border-white/20 px-3 md:px-4 py-1.5 md:py-2 rounded-xl backdrop-blur-md shadow-sm">
                        ...
                        <span className="text-[10px] md:text-xs font-bold text-white uppercase tracking-wider">
                          {todayTrxCount} <span className="hidden sm:inline">Trx Hari Ini</span><span className="sm:hidden">Trx</span>
                        </span>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="bg-gradient-to-r from-blue-600 to-indigo-600 rounded-[1.5rem] md:rounded-[2rem] p-5 md:p-6 shadow-xl shadow-blue-500/20 flex items-center justify-between text-white border border-blue-400/30">
                    <div className="flex items-center gap-4 md:gap-5">
                      ...
                      <div>
                        ...
                        <p className="text-xl md:text-3xl font-black tracking-tighter text-white drop-shadow-sm">
                          {isLoadingHistory ? '—' : `Rp ${totalRevenue.toLocaleString()}`}
                        </p>
                      </div>
                    </div>
                    <div className="text-right flex-shrink-0">
                      <div className="inline-flex items-center gap-1.5 bg-white/10 border border-white/20 px-3 md:px-4 py-1.5 md:py-2 rounded-xl backdrop-blur-md shadow-sm">
                        ...
                        <span className="text-[10px] md:text-xs font-bold text-white uppercase tracking-wider">
                          {isLoadingHistory ? '...' : totalTrx} <span className="hidden sm:inline">Trx Total</span><span className="sm:hidden">Trx</span>
                        </span>
                      </div>
                    </div>
                  </div>
                        ...
                        >
                          <div className="flex items-center gap-4 md:gap-5 w-full md:w-auto">
                            <div className={`w-10 h-10 md:w-12 md:h-12 rounded-xl md:rounded-2xl flex items-center justify-center transition-colors shrink-0 ${
                              expandedId === trx.id ? 'bg-blue-600 text-white' : 'bg-slate-100 text-slate-400'
                            }`}>
            ...
            <div className="flex-1 overflow-y-auto p-4 md:p-8 bg-slate-50/50">
              ...
              {/* Skeleton for rekap tab */}
              {isLoadingHistory && history.length === 0 ? (
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                  {Array.from({ length: 4 }).map((_, i) => (
                    <div key={i} className="bg-white rounded-[2rem] p-8 animate-pulse border border-slate-100">
                ...
                <div ref={reportRef} className="bg-white p-6 sm:p-8 rounded-[2rem] shadow-sm border border-slate-100 mx-auto max-w-3xl">
                  <div className="text-center mb-8 pb-6 border-b border-dashed border-slate-200">
                    ...
                    <p className="text-slate-300 font-bold text-[9px] mt-2 italic">
                      ...
                      >
                        <div>
                          <div className="flex items-center justify-between mb-6 md:mb-8">
                            ...
                            <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest">{item.count} Transaksi</span>
                          </div>
                    ...
                    <div className="mt-8 p-6 md:p-8 bg-slate-900 rounded-[2rem] text-white flex flex-col md:flex-row items-start md:items-center justify-between gap-6 md:gap-8 shadow-xl">
                      ...
                      <div className="text-left md:text-right w-full md:w-auto flex flex-col items-start md:items-end">
                        <p className="text-3xl md:text-4xl font-black tracking-tighter text-blue-400 leading-none break-all">
                          Rp {monthlyRekap.reduce((s, i) => s + i.total, 0).toLocaleString()}
                        </p>
                        <p className="text-sm md:text-base font-black text-emerald-400 tracking-tighter mt-2">
                          Total Laba: Rp {monthlyRekap.reduce((s, i) => s + i.laba, 0).toLocaleString()}
                        </p>
                      </div>
                    </div>
                  )}
